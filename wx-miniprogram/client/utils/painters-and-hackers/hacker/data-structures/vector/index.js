...
import getGridInfo from '../../utils/getGridInfo';

export default {
  namspace: 'vector',
  frameRate: 30,
  frames(imageData, width, height) {
    const {cellRow, cellCol} = getGridInfo(width, height, 20);
      ...
      while (currentCnt < totalCnt) {
        ...
        for (let i = 0; i < appendCnt; i++) {
          const appendToVector = () => {
            ...
            vector.append(getImageData(size + 1));
            return vector;
          };
  ...
  layout(vector, width, height, imageData) {
    const {cellRow, cellCol, cellHeight, cellWidth} = getGridInfo(
      width,
      height,
      20
    );

...
Page({
  ...
  renderImg() {
    fileModel.get(this.data.good.fileid).then(res => {
  ...
  onConfirm(e) {
    ...
    learnModel
      .add(this.data.openid, this.data.did, this.data.type)
      .then(res => {
        this.setData({
          read: true
        });
  ...
  onCancel(e) {
    ...
    learnModel.delete(this.data.openid, this.data.did).then(res => {
      this.setData({
        read: false
      });
      wx.hideLoading();

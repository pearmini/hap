  ...
  while (!sampleSize) {
    ...
    while (queueSize) {
      ...
      for (var j = 0; j < k; ++j) {
        ...
        if (0 <= x && x < width && 0 <= y && y < height && far(x, y)) yield sample(x, y);
      }
...
randomPoissonDisc.metadata = {
  key: "randomPoissonDisc",
  name: "Poisson Disc Random",
  description: "Poisson Disc Random",
  visualizer: random,
};
